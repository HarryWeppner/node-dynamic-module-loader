...
var DynamicModuleLoader = new JS.Class(
    events.EventEmitter,
    {
         ...
         * @param nodeModulesInstallDir The name of the shared subdirectory that will be created. It will be used to
         *                              install the node_modules only once and share them between modules installed
         *                              under the same nodeModulesInstallDir
         * @returns {Future} Future object that will be fulfilled when the module will be loaded
         */
        load:function (moduleName, remoteServerPackageFileExtension, downloadFullUrl, nodeModulesInstallDir)
        {
            var self = this;
            ...
            function checkLocalPackageOrDownload()
            {
                ...
                self.settings.lockManager
                        .obtainExclusiveLock(nodeModulesInstallDir ? nodeModulesInstallDir : moduleName,
            ...
            function hopefullyObtainedLock(err)
            {
                ...
                var nodeModulesInstallationPath;
                if (nodeModulesInstallDir) {
                  moduleInstallationPath = path.join(self.settings.moduleInstallationDir, nodeModulesInstallDir, moduleName);
                  nodeModulesInstallationPath = path.join(self.settings.moduleInstallationDir, nodeModulesInstallDir);
                }
                else {
                  moduleInstallationPath = path.join(self.settings.moduleInstallationDir, moduleName);
                  nodeModulesInstallationPath = moduleInstallationPath;
                }
                    ...
                    fs.exists(self.settings.moduleInstallationDir,
                              function (exists)
                              {
                                  if (exists)
                                  {
                                      if (nodeModulesInstallDir)
                                      {
                                          createNodeModulesInstallationDir();
                                      }
                                      else
                                      {
                                          ensureNpmExists();
                                  ...
                                  else
                                  {
                                      ...
                                      fs.mkdir(self.settings.moduleInstallationDir, 0777, true, wrap(function()
                                        {
                                            if (nodeModulesInstallDir)
                                            {
                                                createNodeModulesInstallationDir();
                ...
                function createNodeModulesInstallationDir()
                {
                    fs.exists(nodeModulesInstallationPath,
                              function (exists)
                              {
                                  if (exists)
                                  {
                                  ...
                                  {
                                      logger.debug(util.format("[dynamic-module-loader] node_modules installation dir %s does not exist. Creating.",
                                          nodeModulesInstallationPath));
                                      fs.mkdir(nodeModulesInstallationPath, 0777, true, wrap(ensureNpmExists));
                                  }
                              });
                }
                ...
                function uncompressTarGzFile(downloadedFile)
                {
                    ...
                    var extractDir = path.join(self.settings.downloadDir, moduleName, modulePackageFileName);

                    // TODO: add top-level module directory via settings
                    var moduleDecompressTargetPath = path.join(extractDir, modulePackageFileName);
                    logger.info(
                        util.format("[dynamic-module-loader] Uncompressing '%s' to '%s'.",
                            downloadedFile,
                ...
                function installDownloadedModule(packageJsonFileLocation)
                {
                    ...
                    else
                    {
                        if (!nodeModulesInstallDir)
                        {
                            logger.info(util.format("[dynamic-module-loader] Installing module %s at location %s.",
                                                    moduleName,
                                                    packageJsonFileLocation));
                            runNpmInstall(packageJsonFileLocation);
                        }
                        else
                        {
                            logger.info(util.format("[dynamic-module-loader] Installing module %s: Copying package.json from %s to %s and doing npm install.",
                                                    moduleName,
                                                    packageJsonFileLocation,
                                                    nodeModulesInstallationPath));

                            fs.readFile(path.join(packageJsonFileLocation, 'package.json'), wrap(function(packageData)
                                {
                                    fs.readFile(path.join(nodeModulesInstallationPath, 'package.json'), function(err, destPackageData)
                                        {
                                            var dependencies1 = JSON.parse(packageData).dependencies;
                                            var dependencies2 = destPackageData ? JSON.parse(destPackageData).dependencies : undefined;
                                            if (_.isEqual(dependencies1, dependencies2))
                                            {
                                                logger.info("[dynamic-module-loader] package.json has not changed, skipping NPM install");
                                                copyNodeModulesInModuleDir(packageJsonFileLocation);
                                                installationComplete(0);
                                            }
                                            else
                                            {
                                                // package.json has changed. Copy it from the module directory into the shared directory, and then run npm intall.
                                                var reader = fs.createReadStream(path.join(packageJsonFileLocation, 'package.json'))
                                                    .on('error', function()
                                                      {
                                                          logger.error("[dynamic-module-loader] Read error while copying package.json: " + err);
                                                          unlockAndFulfill(err, undefined);
                                                      });
                                                var writer = fs.createWriteStream(path.join(nodeModulesInstallationPath, 'package.json'))
                                                    .on('error', function(err)
                                                        {
                                                            logger.error("[dynamic-module-loader] Write error while copying package.json: " + err);
                                                            unlockAndFulfill(err, undefined);
                                                        })
                                                    .on('close', function()
                                                        {
                                                            deleteNodeModulesAndRunNpmInstall(function(returnCode) {
                                                                if (returnCode === 0)
                                                                {
                                                                    copyNodeModulesInModuleDir(packageJsonFileLocation);
                                                                }
                                                                installationComplete(returnCode);
                                                            });
                                                        });
                                                reader.pipe(writer);
                                            }
                        ...
                        function deleteNodeModulesAndRunNpmInstall(callback)
                        {
                            rimraf(path.join(nodeModulesInstallationPath, 'node_modules'), function(err)
                            {
                                if (err)
                                {
                                    logger.error("[dynamic-module-loader] Error while cleaning node_modules: " + err);
                                }
                                runNpmInstall(nodeModulesInstallationPath, callback);
                            })
                        }

                        function runNpmInstall(installDir, callback)
                        {
                            var options = ['install'];
                            options.concat(self.settings.npmOptions);

                            var npm = spawn(self.settings.npmExecutablePath, options, {cwd:installDir});
                    ...
                    function copyNodeModulesInModuleDir(moduleDirPath)
                    {
                        logger.info("[dynamic-module-loader] Copying " + path.join(nodeModulesInstallationPath, 'node_modules') + " to " + moduleInstallationPath);
                        wrench.copyDirSyncRecursive(path.join(nodeModulesInstallationPath, 'node_modules'),
                            path.join(moduleDirPath, 'node_modules'), { forceDelete: true });
                    }
                    ...
                    function installationComplete(code)
                    {
                        ...
                        else
                        {
                            ...
                            if (nodeModulesInstallDir)
                            {
                                fs.unlink(path.join(nodeModulesInstallationPath, 'package.json'), function(err)
                                {
                                    if (err)
                                    {
                                        logger.error('[dynamic-module-loader] error while trying to delete ' + path.join(nodeModulesInstallationPath, 'package.json'));
                                    }
                                    unlockAndFulfill(new Error(message), undefined);
                                });
                            }
                ...
                function unlockAndFulfill(err, module)
                {
                    ...
                    {
                        self.settings.lockManager.releaseExclusiveLock(nodeModulesInstallDir ? nodeModulesInstallDir : moduleName, self.settings.lockOwner)
        ...
        /**
         * Zip an installed module (with the node_modules directory) and give back the path of the archive.
         * @param moduleName name of the module to zip
         * @param nodeModulesInstallDir (optional) name of the sub directory where the module has been installed
         * @return Future A future object that will be fulfill with the archive path once it has been created.
         */
        zipInstalledModule: function(moduleName, nodeModulesInstallDir)
        {
            var self = this;
            var future = new Future();
            var zipName = moduleName + '.zip';
            var moduleInstallationPath;
            if (nodeModulesInstallDir) {
                moduleInstallationPath = path.join(self.settings.moduleInstallationDir, nodeModulesInstallDir, moduleName);
            }
            ...
            logger.info("[dynamic-module-loader] Zipping all files in directory " + moduleInstallationPath);
            var zip = spawn(self.settings.zipExecutablePath, ['-r', zipName, '.', '-i', '*'],
                { cwd: moduleInstallationPath });
            ...
            zip.on('exit', function (code)
            {
                if (code === 0)
                {
                    // Success. Move the zip to ../ (i.e. either shared_dir or moduleInstallationDir)
                    fs.rename(path.join(moduleInstallationPath, zipName),
                              path.join(moduleInstallationPath, '..', zipName), function(err) {
                        future.fulfill(err, path.join(moduleInstallationPath, '..', zipName));
                    });
                }
                else
                {
                    var message = util.format("[dynamic-module-loader] Unable to zip dir '%s'.  Exit code of '%s' was %d. " +
                                              "See log for details.",
                                              moduleInstallationPath,
                                              self.settings.zipExecutablePath,
                                              code);
                    logger.error(message);
                    future.fulfill(new Error(message), undefined);
                }
         ...
         * @param nodeModulesInstallDir name of the sub directory where the module has been installed
         ...
         */
        evict: function (moduleName, nodeModulesInstallDir)
        {
            var self = this;
            var future = new Future();
            self.settings.lockManager
        ...
        __downloadFile:function (sourceUrl, targetFilePath)
        {
            ...
            if(uri.protocol == 'http:' || uri.protocol == 'https:')
            {
                ...
                var request = url.parse(sourceUrl);

                // TODO: check if path ends with `.gz`
                request.headers = {
                    accept: 'application/gzip'
                };

                requester.get(request, function (res) {
        ...
        __decompressTarFile:function (sourceFilePath, targetFilePath)
        {
            ...
            var future = new Future();
            var extractDir = path.dirname(targetFilePath);

            mkdirp(extractDir, function(err) {
                if (err) {
                    logger.error(util.format("[dynamic-module-loader] Error creating directory %s", extractDir));
